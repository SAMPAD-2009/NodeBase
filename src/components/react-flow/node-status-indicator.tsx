  ...
  return (
    <>
      ...
      `}
        ...
        <div className={cn("absolute inset-0 overflow-hidden rounded-[3px]",className)}>
...
const StatusBorder = ({
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -top-[2px] -left-[2px] h-[calc(100%+4px)] w-[calc(100%+4px)] border-3 rounded-sm",
          className,
        )}
      />
      {children}
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case "success":
      return (
        <StatusBorder className={cn("border-green-500/50",className)}>{children}</StatusBorder>
      );
    case "error":
      return <StatusBorder className={cn("border-red-500/50",className)}>{children}</StatusBorder>;
    default:
      return <>{children}</>;
  }
};
